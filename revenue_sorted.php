...
<div>MONTHLY REVENUE SORTING <b><?php echo date('F Y', strtotime($filters));?>
         </b><span style="float:right">Comparison with Previous Month</span><hr>
        <ol>
        <!--For the left side Company List Sort by End Enrolment-->
        <?php               
            $cql = "SELECT company from records WHERE year = '{$filters}' ORDER BY monthly_revenue DESC, company";        
            ...
            foreach($cql_results as $results){?>                
                    <li><?php echo $company = $results['company'];?>
                            ...
                            <?php
                                    ...
                                    $pre_enrolment = "SELECT monthly_revenue FROM records WHERE year = '{$previous_date}' AND company = '$company'";                                    
                                    $sql_previous = mysqli_query($conn, $pre_enrolment);
                                        
                                    while ( $new_rows = mysqli_fetch_assoc($sql_previous) ){
                                        $lastMonth_enrol =  $new_rows['monthly_revenue'];


                                    //present month Ending Enrolment fetch
                                    $endSql = "SELECT monthly_revenue FROM records WHERE year = '{$filters}' AND company = '$company'";
                                    $endResult = mysqli_query($conn, $endSql);

                                    while( $rowEnd = mysqli_fetch_assoc($endResult)){
                                         $currentEnrol = $rowEnd['monthly_revenue'];
                                     };
